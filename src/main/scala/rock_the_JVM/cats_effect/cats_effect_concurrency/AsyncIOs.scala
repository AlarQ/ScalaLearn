...
object AsyncIOs extends IOApp.Simple {
   ...
   */
  def convertFutureToIO[A](future: => Future[A]): IO[A] = IO.async_[A]{(cb:Callback[A]) =>
    future.onComplete{
  ...
  // shorter - by name - we don't want to start future evaluation to early
  def convertFutureToIO_v2[A](future: => Future[A]): IO[A] = IO.async_[A]{(cb:Callback[A]) =>
